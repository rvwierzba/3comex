<?php 
	...
	echo <<<HTML
		...
		<td>{$cp3}</td>		
		<td>
		<a href="#" onclick="editar('{$id}', '{$cp1}', '{$cp2}', '{$cp3}', '{$cp4}', '{$cp5}')" title="Editar Registro"> 
		<i class="bi bi-pencil-square text-primary"></i></a>
		<a href="#" onclick="excluir('{$id}', '{$cp1}')" title="Excluir Registro"> 
		<i class="bi bi-trash text-danger"></i></a>
		<a class="mx-1" href="#" onclick="mostrarDados('{$id}', '{$cp1}', '{$cp2}', '{$cp3}', '{$cp4}', '{$cp5}')" data-target="#modalDados" data-id="<?=$id?>" 
			title="Ver Dados do Registro">
...
<script>
	...
	function mostrarDados(id, cp1, cp2, cp3, cp4, cp5){
		$('#id').val(id);
