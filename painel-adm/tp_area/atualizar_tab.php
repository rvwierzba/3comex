...
$nomeTabela = 'TIPO_AREA_EQUIPAMENTO';
...
if ($response === false) {
    ...
    foreach ($responseArray['dados'] as $item) {
        ...
        $querySelect = $pdo->prepare("SELECT codigo FROM tp_area WHERE codigo = :codigo");
        ...
        if ($total_reg === 0) {
            ...
            $query = $pdo->prepare("INSERT INTO tp_area (codigo, descricao, data_inicio, data_fim, interno_versao, interno_hash) 
    ...
    $pdo->query("DELETE t1 FROM tp_area t1
                 INNER JOIN tp_area t2 
                 WHERE 
...
echo "<br>";
echo "<a class='btn btn-primary' href='index.php?pag=tp_area'>Retornar à Lista de Áreas / Equipamentos</a>";
